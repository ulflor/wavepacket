...
def test_expectation_value():
    ...
    psi_left = wp.builder.product_wave_function(
        grid, [wp.special.Gaussian(-3, fwhm=2.0), lambda x: x]
    )
    psi_right = wp.builder.product_wave_function(
        grid, [wp.special.Gaussian(4, fwhm=2.0), lambda x: x]
    )
...
def _diagonalization_setup() -> wp.operator.OperatorBase:
    grid = wp.grid.Grid(
        [wp.grid.PlaneWaveDof(-10, 10, 5), wp.grid.SphericalHarmonicsDof(6, 1)]
    )
