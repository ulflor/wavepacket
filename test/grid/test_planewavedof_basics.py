...
def test_reject_invalid_constructor_values():
    ...
    with pytest.raises(wp.InvalidValueError):
        wp.grid.PlaneWaveDof(5, 10, 0)

    with pytest.raises(wp.InvalidValueError):
        wp.grid.PlaneWaveDof(5, 10, -1)


def test_dvr_grid():
    dof = wp.grid.PlaneWaveDof(1, 11, 5)
...
def test_fbr_grid():
    interval = 10.0
    dof = wp.grid.PlaneWaveDof(-5, interval - 5, 16)
...
def test_fbr_grid_for_uneven_points():
    ...
    interval = 10.0
    dof = wp.grid.PlaneWaveDof(-5, interval - 5, 15)
