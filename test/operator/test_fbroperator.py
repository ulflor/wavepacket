...
def test_cutoff(grid_1d):
    cutoff = 2
    raw_op = wp.operator.FbrOperator1D(grid_1d, 0, dummy_func)
    cut_op = wp.operator.FbrOperator1D(grid_1d, 0, dummy_func, cutoff=cutoff)
    ...
    assert np.any(raw_values > 1.4 * cutoff)
    assert np.any(raw_values < -1.4 * cutoff)
