...
def test_reject_wrong_number_of_inputs(grid_2d):
    with pytest.raises(wp.InvalidValueError):
        wp.build_product_wave_function(grid_2d, generator)


def test_build_product_state(grid_2d):
    result = wp.build_product_wave_function(grid_2d, [generator, generator], normalize=False)
    ...
    normalized_result = wp.build_product_wave_function(grid_2d, [generator, generator])
...
def test_handle_zero_norm_gracefully(grid_2d):
    result = wp.build_product_wave_function(grid_2d, [zero_generator, zero_generator])
