...
def test_reject_invalid_arguments():
    ...
    with pytest.raises(wp.InvalidValueError):
        wp.builder.random_wave_function(grid, generator, -1.0)

    with pytest.raises(wp.InvalidValueError):
        wp.builder.random_wave_function(grid, generator, 0.0)
...
def test_reproducibility():
    ...
    psi = wp.builder.random_wave_function(grid, generator)
    next_psi = wp.builder.random_wave_function(grid, generator)
    same_psi = wp.builder.random_wave_function(grid, same_generator)
    other_psi = wp.builder.random_wave_function(grid, other_generator)
