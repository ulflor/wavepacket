...
def test_forward_kwargs_to_scipy_solver(grid_1d):
    op = wp.Potential1D(grid_1d, 0, lambda dvr_points: dvr_points)
    eq = wp.SchroedingerEquation(op)
    psi = wp.testing.random_state(grid_1d, 10)

    default_solver = wp.OdeSolver(eq, 5.0)
    kw_solver = wp.OdeSolver(eq, 5.0, method="DOP853", rtol=1e-7, atol=1e-9)
